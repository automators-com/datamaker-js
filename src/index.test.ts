...
test("Basic data generation", async () => {
  const datamaker = new DataMaker({});
  const data = await datamaker.generate({
    quantity: 1,
    fields: [
      {
        name: "first_name",
...
test("Generate data from template in account", async () => {
  const quantity = 2;
  const datamaker = new DataMaker({});
  const result = await datamaker.generateFromTemplateId(
    "clrupt3c7000218qdcaxh9i9i",
    quantity
  );
...
test("Send multiple generated data to API endpoint from account", async () => {
  ...
  const data = await datamaker.generateFromTemplateId(
    "clrupt3c7000218qdcaxh9i9i",
    2
  );
  const result: Data[] = await datamaker.exportToApi(
    "clryyx7f10004ki5yhdbnhv5y",
    data
  );
...
test("Export generated data into DB saved in account", async () => {
  ...
  const data = await datamaker.generateFromTemplateId(
    "cls08ncye0001h7piad743zaa",
    1
  );
  await datamaker.exportToDB("clryufquy0001qvvvb4b6tyzz", "Connection", data);

  for (const entry of data) {
    const deleteConnection = await fetch(
      `https://dev.datamaker.app/api/connections/${entry.id}`,
      {
        method: "DELETE",
        headers: headers,
      }
